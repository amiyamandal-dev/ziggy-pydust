...
```bash
cookiecutter gh:your-org/ziggy-pydust-template
```
...
```bash
cookiecutter /path/to/ziggy-pydust-template --no-input --config-file my-config.yaml
```
...
- [Pydust Documentation](https://pydust.fulcrum.so/)
...
- **Pydust framework**: Visit [Pydust GitHub](https://github.com/fulcrum-so/ziggy-pydust)
